...
describe("App", () => {
  it("should render Monitor and ButtonGroup", () => {
    ...
    expect(wrapper.containsAllMatchingElements([ < Monitor />, < ButtonGroup />
    ]),)
      .to
      .equal(true);
  });
  ...
  it("should start with empty", () => {
    const wrapper = shallow(<App/>);
    expect(wrapper.state("display"))
      .to
      .equal("0");
    expect(wrapper.state("operator"))
      .to
      .equal("");
    expect(wrapper.state("tmpOperator"))
      .to
      .equal("");
    expect(wrapper.state("tmp"))
      .to
      .equal("0");
    expect(wrapper.state("sum"))
      .to
      .equal("");
  });
  ...
  it("click on <plus button> should be plus", () => {
    const wrapper = shallow(<App/>);
    wrapper.setState({operator: "+", tmp: 0, sum: 1});
    wrapper
      .instance()
      .clickHandle("+");
    expect(wrapper.state("sum"))
  ...
  // it("click on <minus button> should change operator to plus", () => {   const
  // wrapper = shallow(<App />);   wrapper.setState({     operator: "=",   });
